...
import { InMemoryAccountRepository, InMemoryAllocationRuleRepository, InMemoryApBillRepository, InMemoryApPaymentRepository, InMemoryArInvoiceRepository, InMemoryArPaymentRepository, InMemoryCostCenterRepository, InMemoryFxRateRepository, InMemoryJournalRepository, InMemoryLedgerSettingsRepository, InMemoryPeriodCloseRunRepository, InMemoryPeriodRepository, InMemoryTaxCodeRepository, } from "../infrastructure/memory.js";
import { InMemoryOutbox } from "../infrastructure/outbox.js";
import { jsonCreated, jsonOk, Router } from "./router.js";
...
export function createFinanceApp() {
    ...
    const periodClose = new PeriodCloseService(periodRepo, closeRunRepo, journalRepo, arInvoiceRepo, apBillRepo, trialBalance, outbox);
    ...
    const allocations = new AllocationService(allocationRepo, accountRepo, costCenterRepo, journalRepo, periodRepo, journals, outbox);
    ...
    // --- Ledger settings ---------------------------------------------------
    router.post("/settings/ledger", async (req) => jsonCreated(await settings.configure(req.ctx, req.body)));
    router.get("/settings/ledger", async (req) => jsonOk(await settings.get(req.ctx)));
    // --- Chart of accounts --------------------------------------------------
    router.post("/accounts", async (req) => jsonCreated((await accounts.createAccount(req.ctx, req.body)).toJSON()));
    ...
    router.get("/accounts/:id", async (req) => jsonOk((await accounts.getAccount(req.ctx, req.params.id)).toJSON()));
    router.post("/accounts/:id/deactivate", async (req) => jsonOk((await accounts.deactivateAccount(req.ctx, req.params.id)).toJSON()));
    router.post("/accounts/:id/reactivate", async (req) => jsonOk((await accounts.reactivateAccount(req.ctx, req.params.id)).toJSON()));
    // --- Journals -----------------------------------------------------------
    router.post("/journals", async (req) => jsonCreated((await journals.createDraft(req.ctx, req.body)).toJSON()));
    ...
    router.get("/journals/:id", async (req) => jsonOk((await journals.getJournal(req.ctx, req.params.id)).toJSON()));
    router.post("/journals/:id/post", async (req) => jsonOk((await journals.post(req.ctx, req.params.id)).toJSON()));
    router.post("/journals/:id/reverse", async (req) => {
        const body = (req.body ?? {});
        const { original, reversal } = await journals.reverse(req.ctx, req.params.id, body);
        return jsonOk({ original: original.toJSON(), reversal: reversal.toJSON() });
    });
    // --- Posting periods & close workflow ------------------------------------
    router.post("/periods", async (req) => jsonCreated((await periods.openPeriod(req.ctx, req.body)).toJSON()));
    ...
    router.get("/periods/:code", async (req) => jsonOk((await periods.getPeriod(req.ctx, req.params.code)).toJSON()));
    router.post("/periods/:code/close/begin", async (req) => jsonOk((await periodClose.beginClose(req.ctx, req.params.code)).toJSON()));
    router.post("/periods/:code/close/run-checks", async (req) => jsonOk((await periodClose.runChecks(req.ctx, req.params.code)).toJSON()));
    router.post("/periods/:code/close/complete", async (req) => jsonOk(await periodClose.completeClose(req.ctx, req.params.code)));
    ...
    router.get("/periods/:code/close-status", async (req) => jsonOk(await periodClose.getCloseStatus(req.ctx, req.params.code)));
    // --- Accounts receivable -------------------------------------------------
    router.post("/ar/invoices", async (req) => jsonCreated((await ar.createInvoice(req.ctx, req.body)).toJSON()));
    ...
    router.get("/ar/invoices/:id", async (req) => jsonOk((await ar.getInvoice(req.ctx, req.params.id)).toJSON()));
    router.post("/ar/invoices/:id/issue", async (req) => jsonOk((await ar.issueInvoice(req.ctx, req.params.id)).toJSON()));
    router.post("/ar/invoices/:id/void", async (req) => {
        const body = (req.body ?? {});
        return jsonOk((await ar.voidInvoice(req.ctx, req.params.id, body.reason ?? "")).toJSON());
    });
    router.post("/ar/payments", async (req) => jsonCreated((await ar.receivePayment(req.ctx, req.body)).toJSON()));
    ...
    router.post("/ar/payments/:id/apply", async (req) => {
        const body = req.body;
        const result = await ar.applyPayment(req.ctx, req.params.id, body.invoiceId, body.amountMinor);
    ...
    // --- Accounts payable ------------------------------------------------------
    router.post("/ap/bills", async (req) => jsonCreated((await ap.createBill(req.ctx, req.body)).toJSON()));
    ...
    router.get("/ap/bills/:id", async (req) => jsonOk((await ap.getBill(req.ctx, req.params.id)).toJSON()));
    router.post("/ap/bills/:id/approve", async (req) => jsonOk((await ap.approveBill(req.ctx, req.params.id)).toJSON()));
    router.post("/ap/bills/:id/void", async (req) => {
        const body = (req.body ?? {});
        return jsonOk((await ap.voidBill(req.ctx, req.params.id, body.reason ?? "")).toJSON());
    });
    router.post("/ap/payments", async (req) => jsonCreated((await ap.issuePayment(req.ctx, req.body)).toJSON()));
    ...
    router.post("/ap/payments/:id/apply", async (req) => {
        const body = req.body;
        const result = await ap.applyPayment(req.ctx, req.params.id, body.billId, body.amountMinor);
    ...
    router.post("/cost-centers", async (req) => jsonCreated((await costCenters.createCostCenter(req.ctx, req.body)).toJSON()));
    router.get("/cost-centers", async (req) => jsonOk((await costCenters.listCostCenters(req.ctx)).map((c) => c.toJSON())));
    router.get("/cost-centers/:id", async (req) => jsonOk((await costCenters.getCostCenter(req.ctx, req.params.id)).toJSON()));
    router.post("/cost-centers/:id/deactivate", async (req) => jsonOk((await costCenters.deactivateCostCenter(req.ctx, req.params.id)).toJSON()));
    router.post("/allocations/rules", async (req) => jsonCreated((await allocations.createRule(req.ctx, req.body)).toJSON()));
    router.get("/allocations/rules", async (req) => jsonOk((await allocations.listRules(req.ctx)).map((r) => r.toJSON())));
    router.get("/allocations/rules/:id", async (req) => jsonOk((await allocations.getRule(req.ctx, req.params.id)).toJSON()));
    router.post("/allocations/rules/:id/deactivate", async (req) => jsonOk((await allocations.deactivateRule(req.ctx, req.params.id)).toJSON()));
    router.post("/allocations/rules/:id/run", async (req) => {
        const body = req.body;
        return jsonOk(await allocations.runRule(req.ctx, req.params.id, body.periodCode));
    });
    // --- Tax codes & FX rates ---------------------------------------------------
    router.post("/tax-codes", async (req) => jsonCreated((await tax.createTaxCode(req.ctx, req.body)).toJSON()));
    router.get("/tax-codes", async (req) => jsonOk((await tax.listTaxCodes(req.ctx)).map((t) => t.toJSON())));
    router.get("/tax-codes/:id", async (req) => jsonOk((await tax.getTaxCode(req.ctx, req.params.id)).toJSON()));
    router.post("/tax-codes/:id/deactivate", async (req) => jsonOk((await tax.deactivateTaxCode(req.ctx, req.params.id)).toJSON()));
    router.post("/fx-rates", async (req) => jsonCreated((await fx.storeRate(req.ctx, req.body)).toJSON()));
