...
import { Journal } from "../domain/journal.js";
...
export class JournalService {
    ...
    periods;
    outbox;
    constructor(journals, accounts, costCenters, periods, outbox) {
    ...
    async createDraft(ctx, command) {
        ...
        for (const line of command.lines) {
            ...
            if (!account.acceptsPostings()) {
                throw new ConflictError(`account ${account.code} (${account.name}) is not postable or inactive`);
            }
            if (account.currency !== command.currency.toUpperCase()) {
                throw new ConflictError(`account ${account.code} is denominated in ${account.currency}, journal is ${command.currency.toUpperCase()}`);
            }
            let costCenterId;
            if (line.costCenterId || line.costCenterCode) {
                ...
                if (!cc.active)
                    throw new ConflictError(`cost center ${cc.code} is inactive`);
                costCenterId = cc.id;
            }
            resolvedLines.push({
    ...
    async post(ctx, journalId) {
        ...
        if (!period)
            throw new NotFoundError("PostingPeriod", journal.periodCode);
        if (!period.acceptsPosting(journal.source)) {
            throw new DomainError(`period ${period.code} is ${period.status} and does not accept ${journal.source} postings`, "PERIOD_NOT_OPEN", 422);
        }
    ...
    async createAndPost(ctx, command) {
     ...
     */
    async reverse(ctx, journalId, options) {
        ...
        if (!period)
            throw new NotFoundError("PostingPeriod (for date)", reversalDate);
        if (!period.acceptsPosting(original.source)) {
            throw new DomainError(`period ${period.code} is ${period.status} and does not accept reversals`, "PERIOD_NOT_OPEN", 422);
        }
