import { actorId, asRecord, dateField, enumField, moneyField, optDateField, optEnumField, optInt, optMoneyField, optNum, optStr, optUlid, requireRole, str, ulidField, ulidParam, } from "../parse.js";
...
const CONTRACT_TYPES = ["permanent", "fixed_term", "contractor", "intern"];
const PAY_FREQUENCIES = ["monthly", "biweekly", "weekly"];
export function registerRequisitionRoutes(router, module) {
    ...
    router.get("/requisitions/:id", (req) => jsonOk(requisitionService.getRequisition(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()));
    router.post("/requisitions/:id/submit", (req) => {
        requireRole(req.ctx, "hr_admin", "manager");
        return jsonOk(requisitionService.submitForApproval(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON());
    });
    router.post("/requisitions/:id/approve", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body ?? {});
        return jsonOk(requisitionService
    ...
    router.post("/requisitions/:id/hold", (req) => {
        requireRole(req.ctx, "hr_admin", "manager");
        const body = asRecord(req.body);
        return jsonOk(requisitionService.hold(req.ctx.tenantId, ulidParam(req.params, "id"), str(body, "note")).toJSON());
    });
    ...
    router.post("/requisitions/:id/cancel", (req) => {
        requireRole(req.ctx, "hr_admin", "manager");
        const body = asRecord(req.body);
        return jsonOk(requisitionService.cancel(req.ctx.tenantId, ulidParam(req.params, "id"), str(body, "note")).toJSON());
    });
...
//# sourceMappingURL=requisition-routes.js.map