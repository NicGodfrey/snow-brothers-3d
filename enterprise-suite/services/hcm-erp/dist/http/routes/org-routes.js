...
import { GRADES } from "../../domain/position.js";
import { asRecord, enumField, optNum, optStr, optUlid, requireRole, str, ulidField, ulidParam, } from "../parse.js";
import { created, jsonOk } from "../router.js";
const ORG_KINDS = ["company", "division", "department", "team"];
export function registerOrgRoutes(router, module) {
    ...
    router.get("/org-units/:id", (req) => jsonOk(orgService.getOrgUnit(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()));
    router.get("/org-units/:id/subtree", (req) => jsonOk({
    ...
    router.patch("/org-units/:id", (req) => {
        ...
        if (name !== undefined)
            unit = orgService.renameOrgUnit(req.ctx.tenantId, id, name);
    ...
    router.post("/org-units/:id/move", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body);
        const unit = orgService.moveOrgUnit(req.ctx.tenantId, ulidParam(req.params, "id"), ulidField(body, "newParentId"));
        return jsonOk(unit.toJSON());
    });
    ...
    router.post("/positions", (req) => {
        ...
        const position = orgService.openPosition(req.ctx.tenantId, {
            ...
            grade: enumField(body, "grade", GRADES),
            fte: optNum(body, "fte"),
    ...
    router.get("/positions", (req) => {
        ...
        const positions = orgService.listPositions(req.ctx.tenantId, orgUnitId ? orgUnitId : undefined);
        const page = normalizePage({
            page: Number(req.query.get("page") ?? 1),
            pageSize: Number(req.query.get("pageSize") ?? 20),
        });
    ...
    router.get("/positions/:id", (req) => jsonOk(orgService.getPosition(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()));
    router.post("/positions/:id/freeze", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body);
        return jsonOk(orgService.freezePosition(req.ctx.tenantId, ulidParam(req.params, "id"), str(body, "reason")).toJSON());
    });
