...
import { actorId, asRecord, dateField, int, num, optBool, optInt, optStr, requireRole, str, ulidField, ulidParam, } from "../parse.js";
...
export function registerLeaveRoutes(router, module) {
    ...
    router.get("/leave-policies", (req) => jsonOk({ items: leaveService.listPolicies(req.ctx.tenantId).map((p) => p.toJSON()) }));
    router.post("/holiday-calendars", (req) => {
        ...
        const holidays = rawHolidays.map((h, i) => {
    ...
    router.get("/leave-requests/:id", (req) => jsonOk(leaveService.getRequest(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()));
    router.get("/employees/:id/leave-requests", (req) => jsonOk({
        items: leaveService
    ...
    router.post("/leave-requests/:id/approve", (req) => {
        ...
        return jsonOk(leaveService.approve(req.ctx.tenantId, ulidParam(req.params, "id"), actorId(req.ctx)).toJSON());
    });
    router.post("/leave-requests/:id/reject", (req) => {
        requireRole(req.ctx, "hr_admin", "manager");
        const body = asRecord(req.body);
        return jsonOk(leaveService
            .reject(req.ctx.tenantId, ulidParam(req.params, "id"), actorId(req.ctx), str(body, "note"))
            .toJSON());
    });
    router.post("/leave-requests/:id/cancel", (req) => jsonOk(leaveService.cancel(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()));
    // ---------------------------------------------------------------- balances
    ...
    router.post("/employees/:id/leave-balances/:leaveType/grant", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body);
        const balance = leaveService.grantAnnualEntitlement(req.ctx.tenantId, ulidParam(req.params, "id"), req.params.leaveType, int(body, "year"));
        return jsonOk(balanceView(balance));
    });
    router.post("/employees/:id/leave-balances/:leaveType/carry-over", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body);
        const days = leaveService.carryOver(req.ctx.tenantId, ulidParam(req.params, "id"), req.params.leaveType, int(body, "fromYear"));
        return jsonOk({ carriedOverDays: days });
    });
    router.post("/leave-accruals/run", (req) => {
        requireRole(req.ctx, "hr_admin");
        const body = asRecord(req.body);
        const count = leaveService.runMonthlyAccrual(req.ctx.tenantId, str(body, "leaveType"), int(body, "year"), int(body, "month"));
