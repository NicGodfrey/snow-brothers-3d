...
export function createHcmModule(overrides) {
    ...
    const contractService = new ContractService(repos.contracts, repos.employees, repos.positions, repos.compensationRecords, outbox, clock);
    const leaveService = new LeaveService(repos.leavePolicies, repos.holidayCalendars, repos.leaveBalances, repos.leaveRequests, repos.employees, outbox, clock);
    const attendanceService = new AttendanceService(repos.attendancePeriods, repos.employees, outbox);
    const compensationService = new CompensationService(repos.compensationRecords, repos.bonuses, repos.employees, outbox);
    const skillsService = new SkillsService(repos.skills, repos.employeeSkills, repos.certifications, repos.employeeCertifications, repos.employees, outbox, clock);
    const requisitionService = new RequisitionService(repos.requisitions, repos.positions, repos.employees, employeeService, contractService, outbox);
    return {
        repos,
        outbox,
        clock,
