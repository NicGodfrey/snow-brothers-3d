...
export class EmploymentContract extends AggregateRoot {
    ...
    static validateTerms(input) {
        const fixedDuration = input.contractType === "fixed_term" || input.contractType === "intern";
        if (fixedDuration && !input.endDate) {
            throw new DomainError(`A ${input.contractType} contract requires an end date`, "CONTRACT_END_DATE_REQUIRED");
        }
        ...
        if (input.endDate && compareDates(input.endDate, input.startDate) <= 0) {
            throw new DomainError(`Contract end date ${input.endDate} must be after start date ${input.startDate}`, "INVALID_CONTRACT_DATES");
        }
        ...
        if (input.weeklyHours <= 0 || input.weeklyHours > MAX_WEEKLY_HOURS) {
            throw new DomainError(`Weekly hours must be in (0, ${MAX_WEEKLY_HOURS}], got ${input.weeklyHours}`, "INVALID_WEEKLY_HOURS");
        }
    ...
    activate() {
        if (this.props.status !== "draft") {
            throw new DomainError(`Only draft contracts can be activated (status: ${this.props.status})`, "CONTRACT_NOT_DRAFT", 409);
        }
    ...
    amend(input) {
        if (this.props.status !== "active") {
            throw new DomainError(`Only active contracts can be amended (status: ${this.props.status})`, "CONTRACT_NOT_ACTIVE", 409);
        }
        const { changes } = input;
        ...
        if (last && compareDates(input.effectiveDate, last.effectiveDate) < 0) {
            throw new DomainError(`Amendment effective date ${input.effectiveDate} precedes previous amendment (${last.effectiveDate})`, "AMENDMENT_OUT_OF_ORDER");
        }
        EmploymentContract.validateTerms({
        ...
        if (changes.baseSalary && changes.baseSalary.currency !== this.props.baseSalary.currency) {
            throw new DomainError(`Amendment cannot change salary currency (${this.props.baseSalary.currency} → ${changes.baseSalary.currency})`, "CURRENCY_MISMATCH");
        }
        const amendment = {
        ...
        if (changes.fte !== undefined)
            this.props.fte = changes.fte;
        if (changes.weeklyHours !== undefined)
            this.props.weeklyHours = changes.weeklyHours;
        if (changes.endDate !== undefined)
            this.props.endDate = changes.endDate;
        if (changes.baseSalary !== undefined)
            this.props.baseSalary = changes.baseSalary;
        this.raise(envelope({
    ...
    terminate(input) {
        if (this.props.status !== "active") {
            throw new DomainError(`Only active contracts can be terminated (status: ${this.props.status})`, "CONTRACT_NOT_ACTIVE", 409);
        }
    ...
    markExpired(asOf) {
        if (this.props.status !== "active")
            return false;
        if (!this.props.endDate || compareDates(asOf, this.props.endDate) <= 0)
            return false;
...
//# sourceMappingURL=employment-contract.js.map