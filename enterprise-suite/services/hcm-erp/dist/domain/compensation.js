import { AggregateRoot, DomainError, envelope, money, nowIso, } from "@enterprise-suite/shared-kernel";
import { compareDates, round2 } from "./common.js";
 ...
 */
export class CompensationRecord extends AggregateRoot {
    constructor(tenantId, props) {
        super(tenantId, props);
    }
    static initialize(tenantId, input) {
    ...
    changeSalary(input) {
        ...
        if (input.newAnnualSalary.currency !== this.props.baseSalary.currency) {
            throw new DomainError(`Salary currency cannot change in a revision (${this.props.baseSalary.currency} → ${input.newAnnualSalary.currency})`, "CURRENCY_MISMATCH");
        }
        ...
        if (last && compareDates(input.effectiveDate, last.effectiveDate) < 0) {
            throw new DomainError(`Salary change effective ${input.effectiveDate} predates the last revision (${last.effectiveDate})`, "REVISION_OUT_OF_ORDER");
        }
        ...
        if ((input.reason === "merit" || input.reason === "promotion") &&
    ...
    addAllowance(input) {
        ...
        if (input.amount.currency !== this.props.baseSalary.currency) {
            throw new DomainError(`Allowance currency must match salary currency (${this.props.baseSalary.currency})`, "CURRENCY_MISMATCH");
        }
...
export class BonusAward extends AggregateRoot {
    constructor(tenantId, props) {
        super(tenantId, props);
    }
    static award(tenantId, input) {
        if (input.amount.amountMinor <= 0) {
...
export function calculatePayslipStub(input) {
    const { record } = input;
    const currency = record.baseSalary.currency;
    ...
    for (const allowance of record.allowances) {
        const amountMinor = allowance.recurrence === "per_pay_period"
    ...
    for (const bonus of input.bonusesDue ?? []) {
        ...
        if (bonus.amount.currency !== record.baseSalary.currency) {
            throw new DomainError(`Bonus currency ${bonus.amount.currency} does not match payroll currency ${currency}`, "CURRENCY_MISMATCH");
        }
        lines.push({
