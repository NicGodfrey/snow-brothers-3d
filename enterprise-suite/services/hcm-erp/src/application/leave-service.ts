...
export const SYSTEM_DECIDER: Ulid = brand<string, "Ulid">("user_system");

export class LeaveService {
  constructor(
    private readonly policies: LeavePolicyRepository,
    private readonly calendars: HolidayCalendarRepository,
  ...
  getPolicy(tenantId: TenantId, leaveType: LeaveTypeCode): LeavePolicy {
    ...
    if (!policy) throw new NotFoundError("LeavePolicy", leaveType);
    return policy;
  }
  ...
  runMonthlyAccrual(tenantId: TenantId, leaveType: LeaveTypeCode, year: number, month: number): number {
    ...
    for (const employee of this.employees.listByTenant(tenantId)) {
      if (!employee.isEmployed()) continue;
      if (compareDates(employee.hireDate, cutoff) > 0) continue;
  ...
  carryOver(tenantId: TenantId, employeeId: Ulid, leaveType: LeaveTypeCode, fromYear: number): number {
    ...
    if (source.pendingDays > 0) {
      throw new ConflictError(
        `Cannot carry over while ${source.pendingDays} day(s) are pending decision for ${fromYear}`,
      );
    }
  ...
  submitRequest(
    ...
    const workingDays = countWorkingDays(
      input.startDate,
      input.endDate,
      this.holidaySetForRange(tenantId, input.startDate, input.endDate),
    );
    ...
    if (overlapping) {
      throw new ConflictError(
        `Request overlaps existing ${overlapping.status} leave ${overlapping.startDate}..${overlapping.endDate}`,
      );
    }
  ...
  cancel(tenantId: TenantId, requestId: Ulid): LeaveRequest {
    ...
    if (wasApproved && compareDates(this.clock.today(), request.startDate) >= 0) {
      throw new DomainError(
        `Leave starting ${request.startDate} has already begun and cannot be cancelled`,
  ...
  private requireBalance(tenantId: TenantId, request: LeaveRequest): LeaveBalance {
    const balance = this.balances.find(
      tenantId,
      request.employeeId,
      request.leaveType,
      yearOf(request.startDate),
    );
    if (!balance) {
  ...
  private holidaySetForRange(tenantId: TenantId, start: IsoDate, end: IsoDate): ReadonlySet<string> {
    const dates = new Set<string>();
    for (let year = yearOf(start); year <= yearOf(end); year += 1) {
      const calendar = this.calendars.findByYear(tenantId, year);
      if (calendar) {
        for (const date of calendar.holidayDates()) dates.add(date);
      }
    }
    return dates;
  }
