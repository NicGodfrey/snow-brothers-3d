...
import { TERMINATION_REASONS, type EmployeeStatus } from "../../domain/employee.js";
...
const STATUSES: readonly EmployeeStatus[] = ["active", "on_leave", "suspended", "terminated"];

export function registerEmployeeRoutes(router: Router, module: HcmModule): void {
  ...
  router.get("/employees", (req) => {
    ...
    const status =
      statusParam && STATUSES.includes(statusParam as EmployeeStatus)
  ...
  router.get("/employees/:id", (req) =>
    jsonOk(employeeService.getEmployee(req.ctx.tenantId, ulidParam(req.params, "id")).toJSON()),
  );

  router.get("/employees/:id/reports", (req) =>
    jsonOk({
  ...
  router.post("/employees/:id/suspend", (req) => {
    ...
    const body = asRecord(req.body);
    return jsonOk(
      employeeService.suspend(req.ctx.tenantId, ulidParam(req.params, "id"), str(body, "reason")).toJSON(),
    );
  });
