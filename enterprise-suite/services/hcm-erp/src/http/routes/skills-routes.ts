...
import { PROFICIENCY_LEVELS, type ProficiencyLevel } from "../../domain/skills.js";
...
function proficiency(value: number): ProficiencyLevel {
  if (!PROFICIENCY_LEVELS.includes(value as ProficiencyLevel)) {
    throw new DomainError(`Proficiency level must be 1-5, got ${value}`, "VALIDATION", 422);
  }
  return value as ProficiencyLevel;
}

export function registerSkillsRoutes(router: Router, module: HcmModule): void {
  ...
  router.get("/skills", (req) =>
    jsonOk({ items: skillsService.listSkills(req.ctx.tenantId).map((s) => s.toJSON()) }),
  );
  ...
  router.get("/employees/:id/skills", (req) =>
    jsonOk({
      items: skillsService
  ...
  router.get("/certifications", (req) =>
    jsonOk({ items: skillsService.listCertifications(req.ctx.tenantId).map((c) => c.toJSON()) }),
  );
  ...
  router.get("/employees/:id/certifications", (req) =>
    jsonOk({
      items: skillsService
