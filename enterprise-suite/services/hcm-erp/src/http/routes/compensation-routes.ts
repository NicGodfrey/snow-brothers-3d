import type { AllowanceRecurrence, BonusKind, SalaryChangeReason } from "../../domain/compensation.js";
...
const CHANGE_REASONS: readonly Exclude<SalaryChangeReason, "initial">[] = [
  "merit",
  "promotion",
  "market_adjustment",
  "demotion",
];
const RECURRENCES: readonly AllowanceRecurrence[] = ["per_pay_period", "annual"];
const BONUS_KINDS: readonly BonusKind[] = ["performance", "signing", "retention", "spot"];

export function registerCompensationRoutes(router: Router, module: HcmModule): void {
  ...
  router.get("/employees/:id/bonuses", (req) =>
    jsonOk({
  ...
  router.get("/employees/:id/payslip-preview", (req) =>
    jsonOk(
      compensationService.payslipPreview(
        req.ctx.tenantId,
        ulidParam(req.params, "id"),
        queryInt(req.query, "year"),
        queryInt(req.query, "month"),
      ),
    ),
  );
}
