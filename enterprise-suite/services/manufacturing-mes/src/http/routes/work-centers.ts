...
export function registerWorkCenterRoutes(router: Router, container: MesContainer): void {
  ...
  router.post("/work-centers/:id/status", async (req) => {
    const body = asRecord(req.body);
    const workCenter = await service.changeStatus(
      req.ctx,
      req.params.id!,
      requireOneOf(body, "status", WORK_CENTER_STATUSES),
  ...
  router.post("/work-centers/:id/calendar", async (req) => {
    const body = asRecord(req.body);
    const workCenter = await service.assignCalendar(
      req.ctx,
      req.params.id!,
      requireString(body, "calendarId"),
    );
    return workCenter.toJSON();
  });
}
