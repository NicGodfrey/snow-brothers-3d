...
import { ROUTING_STATUSES, type RoutingStatus } from "../../domain/routing.js";
import type { OperationInputDto } from "../../application/routing-service.js";
...
function parseOperation(raw: unknown, label: string): OperationInputDto {
...
export function registerRoutingRoutes(router: Router, container: MesContainer): void {
  ...
  router.post("/routings", async (req) => {
    ...
    const routing = await service.create(req.ctx, {
      ...
      operations: optionalArray(body, "operations")?.map((op, i) =>
        parseOperation(op, `operations[${i}]`),
      ),
    });
  ...
  router.get("/routings", async (req) => {
    ...
    if (status && !(ROUTING_STATUSES as readonly string[]).includes(status)) {
      throw new DomainError(
        `status must be one of [${ROUTING_STATUSES.join(", ")}]`,
        "VALIDATION",
        400,
      );
    }
    const routings = await service.list(req.ctx, {
      ...
      status: status as RoutingStatus | undefined,
    });
  ...
  router.post("/routings/:id/operations", async (req) => {
    const routing = await service.addOperation(
      req.ctx,
      req.params.id!,
      parseOperation(req.body, "operation"),
    );
    return { status: 201, body: routing.toJSON() };
  });
...
function seqParam(raw: string): number {
