...
import type { ShiftInput, Weekday } from "../../domain/shift-template.js";
...
export function registerCapacityRoutes(router: Router, container: MesContainer): void {
  ...
  router.post("/shift-templates", async (req) => {
    ...
    const shifts: ShiftInput[] = requireArray(body, "shifts").map((raw, i) => {
      const shift = asRecord(raw, `shifts[${i}]`);
      const days = requireArray(shift, "daysOfWeek").map((d) => {
        if (typeof d !== "number" || !Number.isInteger(d) || d < 0 || d > 6) {
          throw new DomainError(
            `shifts[${i}].daysOfWeek must contain integers 0..6`,
            "VALIDATION",
            400,
          );
        }
...
function rangeFromQuery(query: URLSearchParams): { from: string; to: string } {
