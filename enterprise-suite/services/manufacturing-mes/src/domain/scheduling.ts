...
import { addDays, weekdayOf, type CapacityCalendar } from "./capacity-calendar.js";
 ...
 */
export function windowsFromCalendar(
  template: ShiftTemplate,
  calendar: CapacityCalendar,
): WindowsProvider {
  return (date: string): WorkingWindow[] => {
    const weekday = weekdayOf(date);
    let windows: WorkingWindow[] = template.shifts
    ...
    switch (exception.type) {
      ...
      case "DOWNTIME": {
        ...
        for (let i = windows.length - 1; i >= 0; i -= 1) {
          ...
          if (toTrim === 0 && i > 0) trimmed.unshift(...windows.slice(0, i));
          if (toTrim === 0) break;
        }
        return trimmed;
      }
...
/** Point in working time: a date plus minute-of-day. */
interface TimePoint {
  date: string;
  minute: number;
}

function toIso(point: TimePoint): IsoDateTime {
...
class BackwardCursor {
  ...
  constructor(
    private readonly windows: WindowsProvider,
    dueDate: string,
    private readonly guardDays = DEFAULT_GUARD_DAYS,
  ) {
    ...
    throw new DomainError(
      `No working time found within ${this.guardDays} days before ${dueDate}`,
      "SCHEDULE_NO_CAPACITY",
      422,
    );
  }
  ...
  consume(minutes: number): void {
    ...
    while (remaining > 0) {
      ...
      if (guard > this.guardDays && !advancedWithinDay) {
        throw new DomainError(
          `Ran out of working time scheduling backward (${remaining} min left)`,
          "SCHEDULE_NO_CAPACITY",
          422,
        );
      }
...
class ForwardCursor {
  ...
  constructor(
    ...
    throw new DomainError(
      `No working time found within ${this.guardDays} days after ${earliestDate}`,
      "SCHEDULE_NO_CAPACITY",
      422,
    );
  }
  ...
  consume(minutes: number): void {
    ...
    while (remaining > 0) {
      ...
      if (guard > this.guardDays && !advancedWithinDay) {
        throw new DomainError(
          `Ran out of working time scheduling forward (${remaining} min left)`,
          "SCHEDULE_NO_CAPACITY",
          422,
        );
      }
...
function operationDurations(op: SchedulableOperation, quantity: number) {
  return {
