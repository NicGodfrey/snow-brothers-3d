 ...
 * ("/suppliers/:id/holds"). Handlers receive a parsed JSON body, query params,
 * path params and the tenant context taken from the `x-tenant-id` /
 * `x-user-id` / `x-roles` headers. Domain errors map to the status they
 * declare, so a compliance hold surfaces as a 409 with its hold ids intact.
 */
    ...
    return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      try {
        ...
        if (error instanceof DomainError) {
          ...
          console.error(`[srm-core] unhandled error after ${Date.now() - started}ms`, error);
