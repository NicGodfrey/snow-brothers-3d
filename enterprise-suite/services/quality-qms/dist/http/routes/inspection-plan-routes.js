import { asObject, optionalString, requireEnum, requireObjectArray, requireString, ulidParam, } from "../validation.js";
...
export function registerInspectionPlanRoutes(router, plans) {
    router.post("/inspection-plans", async ({ ctx, body }) => {
        ...
        if (allowedOriginsRaw !== undefined) {
            ...
            allowedOrigins = arr.map((o) => requireEnum({ origin: o }, "origin", ORIGINS));
        }
        ...
        const plan = await plans.createPlan(ctx, {
            ...
            targetType: requireEnum(obj, "targetType", ["material", "product", "process"]),
    ...
    router.get("/inspection-plans", async ({ ctx, query }) => {
        ...
        const items = await plans.listPlans(ctx, {
            status: status && PLAN_STATUSES.includes(status) ? status : undefined,
    ...
    router.post("/inspection-plans/:id/characteristics", async ({ ctx, params, body }) => {
        const { plan, characteristic } = await plans.addCharacteristic(ctx, ulidParam(params, "id"), parseCharacteristic(asObject(body)));
    ...
    router.delete("/inspection-plans/:id/characteristics/:characteristicId", async ({ ctx, params }) => {
        const plan = await plans.removeCharacteristic(ctx, ulidParam(params, "id"), ulidParam(params, "characteristicId"));
        return { body: plan.toJSON() };
    });
...
//# sourceMappingURL=inspection-plan-routes.js.map