import { asObject, optionalBoolean, optionalNumber, optionalString, requireEnum, requireNumber, requireNumberArray, requireString, requireUlid, ulidParam, } from "../validation.js";
const LOT_STATUSES = ["created", "in-progress", "completed", "decided", "cancelled"];
const ORIGINS = ["goods-receipt", "in-process", "final", "customer-return", "stock-audit"];
const DECISIONS = ["accept", "reject", "accept-with-deviation", "partial"];
export function registerInspectionLotRoutes(router, lots) {
    ...
    router.get("/inspection-lots", async ({ ctx, query }) => {
        ...
        const items = await lots.listLots(ctx, {
            status: status && LOT_STATUSES.includes(status) ? status : undefined,
    ...
    router.post("/inspection-lots/:id/results/quantitative", async ({ ctx, params, body }) => {
        const obj = asObject(body);
        const lot = await lots.recordQuantitativeResult(ctx, ulidParam(params, "id"), requireString(obj, "characteristicCode"), requireNumberArray(obj, "readings"), optionalString(obj, "note"));
        return { status: 201, body: lot.toJSON() };
    });
    router.post("/inspection-lots/:id/results/attribute", async ({ ctx, params, body }) => {
        const obj = asObject(body);
        const lot = await lots.recordAttributeResult(ctx, ulidParam(params, "id"), requireString(obj, "characteristicCode"), { inspected: requireNumber(obj, "inspected"), defective: requireNumber(obj, "defective") }, optionalString(obj, "note"));
        return { status: 201, body: lot.toJSON() };
    });
...
//# sourceMappingURL=inspection-lot-routes.js.map