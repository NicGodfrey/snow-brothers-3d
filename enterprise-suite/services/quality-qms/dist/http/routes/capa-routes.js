import { riskRating } from "../../domain/capa.js";
import { asObject, optionalString, optionalUlid, requireEnum, requireIsoDate, requireNumber, requireObjectArray, requireString, ulidParam, } from "../validation.js";
const STATUSES = [
    "draft", "open", "investigation", "action-planning", "implementation", "verification", "closed", "cancelled",
];
export function registerCapaRoutes(router, capas) {
    router.post("/capas", async ({ ctx, body }) => {
        ...
        const capa = await capas.createCapa(ctx, {
            type: requireEnum(obj, "type", ["corrective", "preventive"]),
            title: requireString(obj, "title"),
            description: requireString(obj, "description"),
            priority: requireEnum(obj, "priority", ["low", "medium", "high", "urgent"]),
            owner: optionalString(obj, "owner"),
            riskRating: riskRaw
                ? riskRating(requireNumber(riskRaw, "severity"), requireNumber(riskRaw, "occurrence"), requireNumber(riskRaw, "detection"))
                : undefined,
            source: sourceRaw
                ? {
                    ncrIds: Array.isArray(sourceRaw["ncrIds"])
                        ? sourceRaw["ncrIds"].map((id) => ulidParam({ id }, "id"))
                        : [],
    ...
    router.post("/capas/:id/root-cause", async ({ ctx, params, body }) => {
        ...
        const capa = await capas.recordRootCause(ctx, ulidParam(params, "id"), {
            method: requireEnum(obj, "method", ["5-whys", "fishbone", "8d", "fault-tree", "other"]),
    ...
    router.post("/capas/:id/actions", async ({ ctx, params, body }) => {
        ...
        const { capa, action } = await capas.addAction(ctx, ulidParam(params, "id"), {
            type: requireEnum(obj, "type", ["containment", "corrective", "preventive"]),
            description: requireString(obj, "description"),
            owner: optionalString(obj, "owner"),
    ...
    router.post("/capas/:id/actions/:actionId/complete", async ({ ctx, params, body }) => {
        ...
        const capa = await capas.completeAction(ctx, ulidParam(params, "id"), ulidParam(params, "actionId"), optionalString(obj, "note"));
        return { body: capa.toJSON() };
    });
    router.post("/capas/:id/actions/:actionId/cancel", async ({ ctx, params, body }) => {
        const obj = asObject(body);
        const capa = await capas.cancelAction(ctx, ulidParam(params, "id"), ulidParam(params, "actionId"), requireString(obj, "reason"));
        return { body: capa.toJSON() };
    });
    ...
    router.post("/capas/:id/effectiveness", async ({ ctx, params, body }) => {
        ...
        const capa = await capas.recordEffectiveness(ctx, ulidParam(params, "id"), {
            outcome: requireEnum(obj, "outcome", ["effective", "not-effective"]),
