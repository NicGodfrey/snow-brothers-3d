...
export const SUPPORTED_AQLS = [0.065, 0.1, 0.15, 0.25, 0.4, 0.65, 1.0, 1.5, 2.5, 4.0, 6.5];
...
const FIRST_ZERO_LETTER_BY_AQL = {
    6.5: "A",
    4.0: "B",
    2.5: "C",
    1.5: "D",
...
export function aqlSingleNormalPlan(lotSize, level, aql) {
...
export function determineSampling(rule, lotQuantity) {
    ...
    switch (rule.kind) {
        ...
        case "percentage": {
            ...
            const clamped = Math.min(rule.maximum ?? Number.MAX_SAFE_INTEGER, Math.max(rule.minimum, raw));
...
export function validateSamplingRule(rule) {
    switch (rule.kind) {
        case "full":
            return;
        case "fixed":
        ...
        case "aql":
            if (!SUPPORTED_AQLS.includes(rule.aql)) {
                throw new DomainError(`Unsupported AQL ${rule.aql}; supported: ${SUPPORTED_AQLS.join(", ")}`, "INVALID_SAMPLING_RULE");
            }
            return;
    }
}
