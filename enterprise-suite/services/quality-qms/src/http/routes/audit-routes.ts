...
import type { AnswerType, AuditStatus, AuditType, ItemAnswer } from "../../domain/audit.js";
...
const AUDIT_TYPES: readonly AuditType[] = ["internal", "supplier", "process", "certification"];
const AUDIT_STATUSES: readonly AuditStatus[] = [
  "planned", "in-progress", "review", "completed", "closed", "cancelled",
];
const ANSWER_TYPES: readonly AnswerType[] = ["conformity", "score", "yes-no"];

function parseAnswer(obj: Record<string, unknown>): ItemAnswer {
  const kind = requireEnum(obj, "kind", ANSWER_TYPES);
  switch (kind) {
    case "conformity":
      return {
        kind,
        value: requireEnum(obj, "value", ["conform", "minor-nc", "major-nc", "not-applicable"] as const),
      };
    case "score": {
      const value = optionalNumber(obj, "value");
      if (value === undefined) throw new DomainError("'value' is required for score answers", "VALIDATION");
      return { kind, value };
    }
    case "yes-no":
      return { kind, value: requireEnum(obj, "value", ["yes", "no", "not-applicable"] as const) };
  }
}

export function registerAuditRoutes(router: Router, audits: AuditService): void {
  // --- Templates -----------------------------------------------------------
  ...
  router.post("/audit-templates/:id/sections", async ({ ctx, params, body }) => {
    const obj = asObject(body);
    const { template, section } = await audits.addTemplateSection(
      ctx,
      ulidParam(params, "id"),
  ...
  router.post("/audit-templates/:id/sections/:sectionId/items", async ({ ctx, params, body }) => {
    const obj = asObject(body);
    const { template, item } = await audits.addTemplateItem(
      ctx,
      ulidParam(params, "id"),
      ulidParam(params, "sectionId"),
      {
  ...
  router.get("/audits", async ({ ctx, query }) => {
    ...
    const items = await audits.listAudits(ctx, {
      status:
        status && AUDIT_STATUSES.includes(status as AuditStatus) ? (status as AuditStatus) : undefined,
      auditType:
        auditType && AUDIT_TYPES.includes(auditType as AuditType) ? (auditType as AuditType) : undefined,
  ...
  router.post("/audits/:id/findings", async ({ ctx, params, body }) => {
    ...
    const { audit, finding } = await audits.recordFinding(ctx, ulidParam(params, "id"), {
      classification: requireEnum(obj, "classification", ["observation", "ofi", "minor-nc", "major-nc"] as const),
  ...
  router.post("/audits/:id/findings/:findingId/link", async ({ ctx, params, body }) => {
    const obj = asObject(body);
    const { audit, finding } = await audits.linkFinding(
      ctx,
      ulidParam(params, "id"),
      ulidParam(params, "findingId"),
      { ncrId: optionalUlid(obj, "ncrId"), capaId: optionalUlid(obj, "capaId") },
    );
  ...
  router.post("/audits/:id/complete", async ({ ctx, params, body }) => {
    ...
    const { audit, result } = await audits.completeAudit(
      ctx,
      ulidParam(params, "id"),
      optionalString(obj, "summary"),
    );
