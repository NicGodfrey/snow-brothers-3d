import type { SupplierQualityService } from "../../application/supplier-quality-service.js";
import type { SupplierEventStatus } from "../../domain/supplier-quality.js";
...
const STATUSES: readonly SupplierEventStatus[] = [
  "open", "acknowledged", "in-remediation", "resolved", "written-off",
];

export function registerSupplierQualityRoutes(router: Router, supplierQuality: SupplierQualityService): void {
  router.post("/supplier-quality/events", async ({ ctx, body }) => {
    ...
    const event = await supplierQuality.recordEvent(ctx, {
      ...
      severity: requireEnum(obj, "severity", ["critical", "major", "minor"] as const),
  ...
  router.get("/supplier-quality/events", async ({ ctx, query }) => {
    ...
    const items = await supplierQuality.listEvents(ctx, {
      status:
        status && STATUSES.includes(status as SupplierEventStatus)
          ? (status as SupplierEventStatus)
          : undefined,
  ...
  router.post("/supplier-quality/events/:id/write-off", async ({ ctx, params, body }) => {
    ...
    return {
      body: (
        await supplierQuality.writeOff(ctx, ulidParam(params, "id"), requireString(obj, "reason"))
      ).toJSON(),
    };
  });
