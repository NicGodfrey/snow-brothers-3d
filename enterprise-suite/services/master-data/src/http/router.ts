 ...
 * ("/customers/:id/sites"). Handlers receive a parsed JSON body, query params,
 * path params and the tenant context taken from the `x-tenant-id`,
 * `x-user-id` and `x-roles` headers. Domain errors map to their declared HTTP
    ...
    return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      try {
        ...
        if (error instanceof DomainError) {
          ...
          console.error(`[master-data] unhandled error after ${Date.now() - started}ms`, error);
