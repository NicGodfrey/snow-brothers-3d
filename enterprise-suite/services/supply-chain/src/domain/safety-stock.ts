...
export type SafetyStockMethod =
  | { readonly type: "STATIC"; readonly qty: number }
  | { readonly type: "DAYS_OF_COVER"; readonly days: number }
  | {
...
export function parseSafetyStockMethod(input: unknown): SafetyStockMethod {
  ...
  switch (raw.type) {
    ...
    case "SERVICE_LEVEL": {
      ...
      const stdDev =
        raw.weeklyDemandStdDev === undefined
          ? undefined
...
export function weeklyStdDev(series: readonly number[]): number {
...
export function computeSafetyStock(method: SafetyStockMethod, ctx: SafetyStockContext): number {
  switch (method.type) {
    case "STATIC":
      return method.qty;
    case "DAYS_OF_COVER":
 ...
 * in one place.
 */
export class SafetyStockPolicy extends AggregateRoot<SafetyStockPolicyProps> {
  static create(tenantId: TenantId, input: { name: string; description?: string; method: SafetyStockMethod }): SafetyStockPolicy {
    const name = input.name?.trim();
    if (!name) throw new DomainError("Policy name is required", "VALIDATION");
  ...
  rename(name: string): void {
    ...
    if (!trimmed) throw new DomainError("Policy name is required", "VALIDATION");
