...
export function supplierId(value: string): SupplierId {
  ...
  return brand<string, "SupplierId">(v);
}
...
export function assertQty(name: string, value: unknown, opts?: { allowZero?: boolean }): number {
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throw new DomainError(`${name} must be a finite number`, "VALIDATION");
  }
  if (value < 0) throw new DomainError(`${name} must not be negative`, "VALIDATION");
...
export function assertIntInRange(name: string, value: unknown, min: number, max: number): number {
...
export const UNITS_OF_MEASURE: readonly UnitOfMeasure[] = ["EA", "KG", "L", "M", "BOX"];

export function assertUom(value: unknown): UnitOfMeasure {
  if (typeof value !== "string" || !UNITS_OF_MEASURE.includes(value as UnitOfMeasure)) {
