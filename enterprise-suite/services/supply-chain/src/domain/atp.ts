 ...
 */
export function computeAtp(input: AtpInput): AtpRow[] {
  ...
  let segment = -1;
  for (let t = 0; t < n; t += 1) {
    if (segment + 1 < segmentStarts.length && segmentStarts[segment + 1] === t) segment += 1;
...
export function computeCtp(
  ...
  for (const window of options.capacity) {
    ...
    if (accumulated >= remainder) {
      ...
      return {
        ...
        detail:
          `${bestAtp} from ATP plus ${remainder} from new supply; capacity accumulates by week ` +
  ...
  return {
    ...
    qtyFromNewSupply: 0,
    detail:
      `Cannot promise ${request.qty}: cumulative ATP tops out at ${bestAtp} and supplier capacity ` +
      `covers only ${accumulated} of the remaining ${remainder} within the horizon`,
  };
}

function lastBucketAtOrBefore(rows: readonly AtpRow[], date: IsoDate): number {
  let index = -1;
  for (let t = 0; t < rows.length; t += 1) {
    if (rows[t].weekStart <= date) index = t;
    else break;
  }
  return index;
}
...
function pad(values: readonly number[], length: number): number[] {
