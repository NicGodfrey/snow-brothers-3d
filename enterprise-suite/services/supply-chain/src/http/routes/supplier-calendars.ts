...
export function registerSupplierCalendarRoutes(router: Router, module: SupplyChainModule): void {
  ...
  router.get("/supplier-calendars/:id", async (req) => {
    const calendar = await module.capacity.getCalendar(req.ctx, req.params.id as Ulid);
  ...
  router.get("/supplier-capacity-load", async (req) => {
    ...
    if (!supplier) throw new DomainError("supplierId query parameter is required", "VALIDATION");
