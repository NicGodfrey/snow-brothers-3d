...
import { bucketize, makeWeeklyCalendar, type PlanningCalendar } from "../domain/calendar.js";
...
import { netItem, type MrpException } from "../domain/mrp.js";
import { effectiveQtyPer, type PlanningItem } from "../domain/planning-item.js";
import { PlanningRun } from "../domain/planning-run.js";
import { weeklyStdDev, type SafetyStockContext } from "../domain/safety-stock.js";
import { SupplyPlan } from "../domain/supply-plan.js";
import { roundQty, type LocationCode } from "../domain/types.js";
...
export class PlanningService {
  ...
  async getRun(ctx: TenantContext, id: Ulid): Promise<PlanningRun> {
    const run = await this.deps.runs.findById(ctx.tenantId, id);
    if (!run) throw new NotFoundError("PlanningRun", id);
    return run;
  }

  async listRuns(ctx: TenantContext): Promise<PlanningRun[]> {
   ...
   */
  async executeRun(ctx: TenantContext, runId: Ulid): Promise<PlanningRun> {
  ...
  private async plan(
    ctx: TenantContext,
    run: PlanningRun,
  ): Promise<{ itemsPlanned: number; ordersCreated: number; exceptionCount: number; levelsProcessed: number }> {
    ...
    const explodedDemand = new Map<string, number[]>();
    ...
    for (let level = 0; level < levels.length; level += 1) {
      for (const item of levels[level]) {
        ...
        const exceptions: MrpException[] = [...result.exceptions];
  ...
  private async resolveScope(ctx: TenantContext, run: PlanningRun): Promise<PlanningItem[]> {
    const active = await this.deps.items.listActive(ctx.tenantId);
    if (run.scope.type === "ALL_ITEMS") return active;

    const bySku = new Map(active.map((item) => [item.sku, item]));
    const selected = new Map<string, PlanningItem>();
    const queue = [...run.scope.skus];
    while (queue.length > 0) {
      const sku = queue.pop() as string;
      ...
      for (const line of item.bom) queue.push(line.componentSku);
    }
    return [...selected.values()];
  }

  /** Independent demand (residual forecast + allocations) plus exploded dependent demand. */
  private async grossRequirements(
    ctx: TenantContext,
    run: PlanningRun,
    item: PlanningItem,
    calendar: PlanningCalendar,
    exploded: number[] | undefined,
  ): Promise<number[]> {
    ...
    return calendar.weekStarts.map((_, i) =>
      roundQty(residual[i] + actualsByBucket[i] + (exploded?.[i] ?? 0)),
    );
  }

  /** Open external receipts plus firmed planned orders from the item's previous plan. */
  private async scheduledReceipts(
    ctx: TenantContext,
    ...
    if (previousPlan) {
      const firmed = bucketize(
        calendar,
        previousPlan.firmedOrders().map((o) => ({ date: o.dueDate, qty: o.qty })),
        { includePastDueInFirstBucket: true },
      );
      for (let i = 0; i < totals.length; i += 1) totals[i] = roundQty(totals[i] + firmed[i]);
    }
    return totals;
  }

  private async safetyStockFor(
    ctx: TenantContext,
    run: PlanningRun,
    item: PlanningItem,
    gross: readonly number[],
  ): Promise<number> {
    if (!item.safetyStockPolicyId) return 0;
   ...
   */
  private async checkSupplierCapacity(
    ctx: TenantContext,
    run: PlanningRun,
    item: PlanningItem,
    calendar: PlanningCalendar,
    proposals: readonly { dueIndex: number; qty: number }[],
  ): Promise<MrpException[]> {
    if (!item.preferredSupplierId || proposals.length === 0) return [];
    const supplierCalendar = await this.deps.calendars.findFor(ctx.tenantId, item.preferredSupplierId, item.sku);
    if (!supplierCalendar) return [];

    const byWeek = new Array<number>(calendar.weekCount).fill(0);
    for (const proposal of proposals) byWeek[proposal.dueIndex] += proposal.qty;

    const exceptions: MrpException[] = [];
    for (let t = 0; t < calendar.weekCount; t += 1) {
      if (byWeek[t] <= 0) continue;
      const capacity = supplierCalendar.capacityFor(calendar.weekStarts[t]);
      if (byWeek[t] > capacity) {
        const message =
          `${item.sku}: planned purchases of ${roundQty(byWeek[t])} in week ${calendar.weekStarts[t]} exceed ` +
  ...
  private explodeIntoComponents(
    ...
    item: PlanningItem,
    calendar: PlanningCalendar,
    proposals: readonly { releaseIndex: number; qty: number }[],
    bySku: Map<string, PlanningItem>,
    explodedDemand: Map<string, number[]>,
  ): void {
    if (item.bom.length === 0 || proposals.length === 0) return;
    for (const line of item.bom) {
      ...
      const target = explodedDemand.get(line.componentSku) ?? new Array<number>(calendar.weekCount).fill(0);
      for (const proposal of proposals) {
        target[proposal.releaseIndex] = roundQty(
          target[proposal.releaseIndex] + proposal.qty * effectiveQtyPer(line),
        );
      }
