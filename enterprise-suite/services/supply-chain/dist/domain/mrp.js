 ...
 */
function feasibleProjection(input, gross, scheduled, plannedOrders, leadTimeBuckets) {
    const n = gross.length;
    const receipts = new Array(n).fill(0);
...
function detectStockExceptions(input, feasibleOnHand, exceptions) {
    for (let t = 0; t < feasibleOnHand.length; t += 1) {
        ...
        if (feasibleOnHand[t] < 0) {
            exceptions.push({
                ...
                severity: "ERROR",
                weekStart,
                message: `${input.sku}: projected on-hand ${feasibleOnHand[t]} in week ${weekStart} is negative ` +
            ...
            exceptions.push({
                ...
                severity: "WARNING",
                weekStart,
                message: `${input.sku}: projected on-hand ${feasibleOnHand[t]} in week ${weekStart} ` +
 ...
 * new expedited order.
 */
function detectExpediteOpportunity(input, feasibleOnHand, scheduled, exceptions) {
    const firstProblem = feasibleOnHand.findIndex((balance) => balance < 0 || (input.safetyStock > 0 && balance < input.safetyStock));
    if (firstProblem < 0)
        return;
 ...
 */
function detectExcessReceipts(input, rows, scheduled, exceptions) {
