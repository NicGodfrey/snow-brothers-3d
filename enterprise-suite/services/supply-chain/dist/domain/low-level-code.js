 ...
 */
export function computeLowLevelCodes(items) {
    const bySku = new Map();
    for (const item of items)
        bySku.set(item.sku, item);
    const codes = new Map();
    const onStack = new Set();
    function visit(sku, level, path) {
        if (onStack.has(sku)) {
        ...
        if (!item)
            return; // component not under planning control; leaf by definition
        onStack.add(sku);
    ...
    for (const item of items) {
        for (const line of item.bom)
            componentSkus.add(line.componentSku);
    }
    for (const item of items) {
        if (!componentSkus.has(item.sku))
            visit(item.sku, 0, []);
    }
    ...
    for (const item of items) {
        if (!codes.has(item.sku))
            visit(item.sku, 0, []);
    }
    return codes;
}
/** Items grouped by low-level code, ascending (level 0 first). */
export function itemsByLevel(items, codes) {
...
//# sourceMappingURL=low-level-code.js.map