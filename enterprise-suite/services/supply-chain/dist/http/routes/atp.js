...
import { asObject, optionalBoolean, optionalString, queryInt, requireLocation, requireNumber, requireString, } from "../validate.js";
const DEMAND_REF_TYPES = ["SALES_ORDER", "TRANSFER_ORDER", "MANUAL"];
const RECEIPT_SOURCE_TYPES = ["PURCHASE_ORDER", "WORK_ORDER", "TRANSFER_ORDER"];
export function registerAtpRoutes(router, module) {
    router.get("/atp", async (req) => {
        const sku = req.query.sku;
        if (!sku)
            throw new DomainError("sku query parameter is required", "VALIDATION");
        const rows = await module.atp.atpReport(req.ctx, sku, requireLocation(req.query), queryInt(req.query, "weeks", 12, 1, 104));
    ...
    router.post("/allocations/:id/cancel", async (req) => {
        const allocation = await module.atp.cancelAllocation(req.ctx, req.params.id);
    ...
    router.get("/inventory", async (req) => {
        const records = await module.deps.inventory.list(req.ctx.tenantId, req.query.location ? locationCode(req.query.location) : undefined);
    ...
    router.get("/scheduled-receipts", async (req) => {
        const receipts = await module.deps.receipts.list(req.ctx.tenantId, req.query.location ? locationCode(req.query.location) : undefined);
    ...
    router.delete("/scheduled-receipts/:id", async (req) => {
        await module.atp.removeScheduledReceipt(req.ctx, req.params.id);
        return { status: 204, body: null };
    });
}
