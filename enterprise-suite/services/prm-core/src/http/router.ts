 ...
 *
 * Patterns are static segments plus `:param` captures ("/partners/:id/tier").
 * Handlers receive the parsed JSON body, query parameters, path parameters and
 * the tenant context lifted from `x-tenant-id` / `x-user-id` / `x-roles`.
 * Domain errors map to their declared HTTP status.
 */
    ...
    return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      try {
        ...
        if (error instanceof DomainError) {
          ...
          console.error(`[prm-core] unhandled error after ${Date.now() - started}ms`, error);
