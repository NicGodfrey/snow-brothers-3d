 ...
 * ("/deal-registrations/:id/approve"). Handlers receive a parsed JSON body,
 * query params, path params and the tenant context taken from the
 * `x-tenant-id` / `x-user-id` / `x-roles` headers. Domain errors map to the
 * status they declare, so route handlers never translate errors themselves.
 */
    ...
    return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      ...
      try {
        ...
        if (error instanceof DomainError) {
          ...
          console.error(`[channel-prm] unhandled error after ${Date.now() - started}ms`, error);
